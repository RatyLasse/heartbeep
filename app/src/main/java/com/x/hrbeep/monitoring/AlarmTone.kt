...
internal object AlarmTone {
    ...
    private const val UPPER_FREQUENCY_HZ = 880.0
    private const val LOWER_FREQUENCY_HZ = 659.25
    ...
    fun buildSamples(spec: AlarmToneSpec): ShortArray {
        ...
        val releaseSamples = ((spec.sampleRateHz * spec.releaseMs) / 1_000).coerceAtLeast(1)
        ...
        return ShortArray(sampleCount) { index ->
            val envelope = when {
                ...
                index >= sampleCount - releaseSamples ->
                    (sampleCount - index - 1).coerceAtLeast(0) / releaseSamples.toDouble()
